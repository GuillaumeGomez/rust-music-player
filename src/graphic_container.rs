...
pub struct GraphicContainer {
    pub buttons: Vec<GraphicButton>,
    pub widgets: Vec<GraphicElement + 'static>,
...
impl GraphicContainer {
    ...
    pub fn add_tab(&self, label: &str, widget: GraphicElement) {
    	let new_width = self.get_size().x / self.buttons.len() as f32 + 1f32;
	...
	for it in self.buttons.mut_iter() {
    	...
    	let mut tmp = GraphicElement::new_init(&Vector2f{x: self.get_size().x / self.buttons.len() as f32 + 1f32, y: 25f32},
	    &Vector2f{x: pos, y: self.get_position().y});

	tmp.set_label(label);
	self.buttons.push(tmp);
...
impl GraphicElement for GraphicContainer {
        ...
        if position.y <= self.get_position().y + 25f32 {
            ...
            for it in self.buttons.mut_iter() {
                if it.is_inside(position) {
                    ...
                    self.buttons[self.current_tab].clicked(position);
                    it.clicked(position);
                    self.current_tab = pos;
                    self.widgets[self.current_tab].need_to_draw = true;
                }
                pos += 1;
            }
        } else {
            self.widgets[self.current_tab].clicked(position);
        }
    }
    ...
    fn draw(&mut self, win: &mut RenderWindow) {
        ...
        win.draw(&self.widgets[self.current_tab]);
    }
